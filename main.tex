...
\node[store, fill=green!8, right=of parse, yshift=0.8cm] (sqlite) {SQLite\\Analytics};
\node[box, fill=orange!10, right=of parse, yshift=-0.8cm] (embed) {Qwen3\\Embedding};
\node[store, fill=green!8, right=of embed] (milvus) {Milvus\\Dense + BM25};

\node[box, fill=purple!8, below=1.6cm of parse] (ui) {Streamlit UI\\7 tabs};
\node[box, fill=purple!8, right=of ui] (api) {FastAPI\\Backend};
...
\draw[arrow] (sqlite.south) |- (api.north);
\draw[arrow] (retrieval) -- (rerank);
\draw[arrow] (rerank) -- (llm);
\draw[arrow] (llm.south) |- (ui.south);
\end{tikzpicture}
...
\toprule
\textbf{Component} & \textbf{Mode} & \textbf{Value/Count} & \textbf{p50} & \textbf{p95} & \textbf{Mean}\\
\midrule
...
Giới hạn thứ hai là ground-truth retrieval hiện là weak ground-truth từ title-based known-item cases. Cách này phù hợp cho regression test tự động và kiểm tra khả năng tìm đúng paper, nhưng chưa đo đầy đủ relevance của các truy vấn literature review rộng. Trong tương lai, cần xây dựng bộ ground-truth thủ công hoặc semi-automatic dựa trên chuyên gia, citation graph, S2ORC/OpenAlex/Semantic Scholar, hoặc BEIR/SciDocs để đánh giá semantic relevance.
...
Giới hạn thứ tư là một số model-stage benchmark chưa hoàn thành do ràng buộc runtime. BAAI/bge-m3 thất bại vì yêu cầu phiên bản Torch an toàn hơn cho \texttt{torch.load}; Alibaba-NLP/gte-Qwen2-7B-instruct bị timeout khi tải shard lớn. Những kết quả này nên được đọc như runtime stability findings, không phải kết luận model-quality.
...
Các artifact đánh giá cuối cùng nằm tại \path{data/eval/runs/final_coverage_20260428T175804Z}. Gói báo cáo tổng hợp gồm Excel và phần IEEE evaluation chi tiết nằm trong \path{arxiv_copilot_final_report_bundle.zip}. Folder LaTeX này được thiết kế theo hướng IEEE Conference template~\cite{ieeeTemplates2024}, upload trực tiếp lên Overleaf và compile bằng XeLaTeX.
